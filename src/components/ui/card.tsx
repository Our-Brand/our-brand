...
const cardVariants = cva(
  ['relative rounded-2xl', 'bg-white/5 backdrop-blur-md', 'ring-2', 'transition-all duration-300 ease-out'].join(' '),
  {
    variants: {
      variant: {
        core: 'ring-white/15 shadow-[0_0_20px_rgba(255,255,255,0.08)]',
        pro: 'ring-cyan-400/70 shadow-[0_0_30px_rgba(34,211,238,0.25)]',
        onDemand: 'ring-fuchsia-500/60 shadow-[0_0_30px_rgba(217,70,239,0.18)]'
      },
      interactive: {
        true: '',
...
const Card = forwardRef<HTMLDivElement, CardProps>(({ className, variant, interactive, asChild, ...props }, ref) => {
  const Comp = asChild ? Slot : 'div';

  return <Comp ref={ref} className={cn(cardVariants({ variant, interactive }), className)} {...props} />;
});
...
const CardTitle = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => (
  <p ref={ref} className={cn('text-base md:text-lg font-semibold tracking-[0.20em] text-white/80', className)} {...props} />
));
CardTitle.displayName = 'CardTitle';

const CardPrice = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => (
  <p ref={ref} className={cn('mt-3 text-3xl md:text-4xl font-semibold text-white', className)} {...props} />
));
  ...
  return (
    <div
      className={[
        ...
        featured ? 'ring-cyan-400/40 shadow-[0_0_20px_rgba(34,211,238,0.15)]' : 'ring-white/10 shadow-[0_0_20px_rgba(255,255,255,0.06)]'
      ].join(' ')}>
...
export function PackageCard({ pkg, t }: { pkg: Package; t: (key: string) => string }) {
  const { user } = useAuth();
  ...
  const blocks: Array<{ type: 'spacer' } | { type: 'p'; text: string } | { type: 'ul'; items: string[] }> = [];
    ...
    <Card
      ...
      <div className="flex h-full flex-col items-center text-center">
        <CardHeader className="shrink-0">
          ...
          <CardPrice>{t(pkg.price)}</CardPrice>
        </CardHeader>

        <CardContent
          className={['w-full', 'text-start', 'font-medium', 'text-white/80', 'flex-1', 'overflow-y-auto', 'pr-2', 'space-y-4'].join(' ')}>
          {blocks.map((b, idx) => {
              ...
              <ul key={idx} className="mt-1 space-y-2 list-none pl-0">
                {b.items.map((item) => (
                  <li key={item} className="flex gap-3">
                    ...
                    <span className="text-sm leading-relaxed">{renderFirstSentenceBold(item)}</span>
                  </li>
                ))}
              </ul>
            );
      ...
      <div className="relative flex h-full items-start gap-3">
        ...
        <div className="min-w-0 flex h-full flex-col">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-muted-foreground">{index.toString().padStart(2, '0')}</span>
            <h4 className="truncate text-base font-semibold">{title}</h4>
          </div>

          <p className="mt-2 text-sm leading-relaxed text-muted-foreground line-clamp-3 flex-1">{description}</p>
...
export { Card, CardHeader, CardTitle, CardPrice, CardContent, CardDescription, CardFooter, SkeletonCard, StepCard };
